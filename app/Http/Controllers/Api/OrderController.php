...
class OrderController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $order = $user->orders()->create(['vat_rate' => $tax_rates, 'vat' =>  Settings::findOrFail(1)->vat, 'sub_total' =>  $sub_total, 'total' => $total, 'city_id' => $request->city_id , 'sort' => 'الدفع عند الإستلام' , 'status' => 'تم الإستلام']);
        ...
        foreach($cart as $item){
            ...
            $order->items()->create(['product_id' => $item->product_id, 'price' => $item->product_price, 'qty' => $item->product_qty, 'options' => $item->product_options]);
        }
