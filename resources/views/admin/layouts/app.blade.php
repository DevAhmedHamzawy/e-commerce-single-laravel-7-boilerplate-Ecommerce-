...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="{{ url('admin/panel/css/style.css') }}">
...
<body>
  <nav class="navbar navbar-expand-lg fixed-top">
    ...
    <a class="navbar-brand" href="#"><img src="{{ url('admin/panel/img/logo.png') }}"  height="40" style="margin-right: 0.9em;" /></a>
    ...
    <div class="collapse navbar-collapse" id="navbarSupportedContent">
        ...
        <li class="nav-item dropdown notifications-menu">
          <a class="nav-link dropdown-toggle" href="http://example.com" id="navbarDropdownMenuLink" data-toggle="dropdown" data-target="#messages" aria-haspopup="true" aria-expanded="false">
            ...
            <span class="label label-warning bg-warning" id="the_number_of_orders">{{ count(\App\Order::whereReadAt(null)->get()) }}</span>
          </a>
          <div id="messages" class="dropdown-menu" aria-labelledby="navbarDropdownMenuLink" style="margin-right: -80px;">
            <ul class="dropdown-menu-over list-unstyled">
              <li class="header-ul text-center">
                <a href="{{ route('orders.index') }}">
                  لديك {{ count(\App\Order::whereReadAt(null)->get()) }} طلبات جديدة
                </a>
              </li>
              <li>
                <!-- inner menu: contains the actual data -->
                <ul class="menu list-unstyled">
                  <li>
                    @foreach(\App\Order::whereReadAt(null)->get() as $a)
                    <a href="{{ route('orders.show', $a->id) }}">
                      <i class="fa fa-users text-aqua"></i> {{ $a->id }}
                    </a>
                    <hr>
                    @endforeach
                  </li>
              ...
              <li class="footer-ul text-center"><a href="{{ route('orders.index') }}">مشاهدة الكل</a></li>
            </ul>
          </div>
        </li>
            ...
            <div class="sidebar">
                <ul class="navbar-nav">
                  ...
                  <li class="nav-item"> 
                    ...
                    <li class="nav-item"><a href="{{ url('admin/partners') }}" class="nav-link px-2"><i class="material-icons icon">collections</i><span class="text">الرعاة</span></a></li>
                   
                    <li class="nav-item"><a href="{{ url('admin/cities') }}" class="nav-link px-2"><i class="material-icons icon">place</i><span class="text text-white"> المدن </span></a></li>  

                    <li class="nav-item"><a href="{{ url('admin/coupons') }}" class="nav-link px-2"><i class="material-icons icon">payment</i><span class="text text-white"> الكوبونات </span></a></li>   

                    <li class="nav-item"><a href="{{ url('admin/contacts') }}" class="nav-link px-2"><i class="material-icons icon">collections</i><span class="text">رسائل التواصل</span></a></li>
...
<script src="{{ url('admin/panel/js/script.js') }}"></script>
...
<script>
  ...
  $(document).ready(function(){
    ...
    var n = parseInt($("#the_number_of_orders").text());
  ...
  function myFunction(n) {
      setInterval(function(){ 
        ...
        axios.get('get_orders_no').then((res) => { if(res.data > n) { beep(); $("#the_number_of_orders").text(res.data); }  }) 

      }, 10000);
  }

  function beep() {
    var snd = new Audio("https://audio-previews.elements.envatousercontent.com/files/148785970/preview.mp3");  
    snd.play();
  }
